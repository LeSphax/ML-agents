...
class PPOTrainer(Trainer):
    ...
    def take_action(self, all_brain_info: AllBrainInfo):
        ...
        # print("State", curr_brain_info.vector_observations)
        run_out = dict(zip(run_list, values))
        # print("Value", run_out[self.model.value])
        # print("Value mean", run_out[self.model.value].mean())
    ...
    def process_experiences(self, all_info: AllBrainInfo):
        ...
        for l in range(len(info.agents)):
            ...
            if ((info.local_done[l] or len(agent_actions) > self.trainer_parameters['time_horizon'])
                ...
                # print("States: ", self.training_buffer[info.agents[l]]['states'].get_batch())
                # print("Values: ",self.training_buffer[info.agents[l]]['value_estimates'].get_batch())
                # print("Rewards: ",self.training_buffer[info.agents[l]]['rewards'].get_batch())
                # print("Actions: ",self.training_buffer[info.agents[l]]['actions'].get_batch())
                ...
                # print(self.training_buffer[agent_id]['advantages'])
                # print("Advantages", self.training_buffer[agent_id]['advantages'].get_batch())
                # print(self.training_buffer[agent_id]['value_estimates'])
                # print("Values", self.training_buffer[agent_id]['value_estimates'].get_batch())
    ...
    def update_model(self):
        ...
        for k in range(num_epoch):
            ...
            for l in range(len(self.training_buffer.update_buffer['actions']) // n_sequences):
                ...
                # print("Returns holder", len(self.sess.run(self.model.returns_holder, feed_dict=feed_dict)))
                # print("Advantage", len(self.sess.run(self.model.advantage, feed_dict=feed_dict)))
                # print("Value", self.sess.run(self.model.value, feed_dict=feed_dict)[0:10], len(self.sess.run(self.model.value, feed_dict=feed_dict)))
                # print("Value Reduced", self.sess.run(tf.reduce_sum(self.model.value, axis=1), feed_dict=feed_dict)[0:10], len(self.sess.run(tf.reduce_sum(self.model.value, axis=1), feed_dict=feed_dict)))
                # print("Old value",  self.sess.run(self.model.old_value, feed_dict=feed_dict)[0:10], len(self.sess.run(self.model.old_value, feed_dict=feed_dict)))
                # decay_epsilon = tf.train.polynomial_decay(0.2, self.model.global_step, 3e5, 0.1, power=1.0)
                # clipped_value_estimate = self.model.old_value + tf.clip_by_value(tf.reduce_sum(self.model.value, axis=1) - self.model.old_value,
                #                                                    - decay_epsilon, decay_epsilon)
                # print("Clipped VE",  self.sess.run(clipped_value_estimate, feed_dict=feed_dict)[0:10], len(self.sess.run(clipped_value_estimate, feed_dict=feed_dict)))
                # v_opt_a = tf.squared_difference(self.model.returns_holder, tf.reduce_sum(self.model.value, axis=1))
                # v_opt_b = tf.squared_difference(self.model.returns_holder, clipped_value_estimate)
                # print("opta",  self.sess.run(v_opt_a, feed_dict=feed_dict)[0:10], len(self.sess.run(v_opt_a, feed_dict=feed_dict)))
                # print("opt b",  self.sess.run(v_opt_b, feed_dict=feed_dict)[0:10], len(self.sess.run(v_opt_b, feed_dict=feed_dict)))
                # print("Value Loss",  self.sess.run(self.model.value_loss, feed_dict=feed_dict))
             
                # print("Log sigma",  self.sess.run(self.model.log_sigma_sq, feed_dict=feed_dict)[:10])
                # print("Sigma ",  self.sess.run(self.model.sigma_sq, feed_dict=feed_dict)[:10])
                # print("Entropy",  self.sess.run(self.model.entropy, feed_dict=feed_dict)[:10])
